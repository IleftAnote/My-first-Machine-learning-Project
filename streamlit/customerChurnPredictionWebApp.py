...
if app_mode=='Home': 
    ...
    def barandhistogram():
        st.header('Occupation Churn Distribution')
        fig, (ax_hist, ax_kde) = plt.subplots(
            nrows = 1,
            ncols = 2,
            figsize = (6, 4))
        sns.histplot(
            data = data, x = "churn", hue = "occupation", multiple = "stack", ax = ax_hist)
        sns.kdeplot(
            data = data, x = "churn", hue = "occupation", multiple = "stack", ax = ax_kde)
        
        ax_hist.set_title("Churn Rate in Occupation Feature!")
        ax_kde.set_title("Histogram on the Same")
        ax_kde.grid(True)
...
# Sidebar setup
    ...
    options = st.sidebar.radio('Select what you want to display:', [ 'Data Summary', 'Data Header', 'Histogram', 'Scatter Plot', 'Fancy Plots'])
